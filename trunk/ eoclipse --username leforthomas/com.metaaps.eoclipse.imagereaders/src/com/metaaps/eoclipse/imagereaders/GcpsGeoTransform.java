 ...
 * @author thoorfr
 */
public class GcpsGeoTransform implements IGeoTransform {
    ...
    public GcpsGeoTransform(List<GCP> gcps, String epsgGeoProj) {
        ...
        for (GCP gcp : gcps) {
            ...
            System.out.println(src[i]+"---"+dst[i]);
            i++;            
        }
        if(gcps.size()<6){
            Point2D[] srctemp=src;
        ...
        //AG modify from 2 to 3
        pix2geo = new WarpTransform2D(src, dst, i < 6 ? 1 : 3);
        geo2pix = new WarpTransform2D(dst, src, i < 6 ? 1 : 3);
